...
import BussinessProfile from "../models/bussinessModel";
...
export async function createBussinessProfile(req,res){
  try {
    ...
    const profile = new BusinessProfile({
      ...
      return res.status(201).json({
        success:true,
        data:saved,
        message:"Bussiness Profile Created"
      })

    
  } 
  catch (err) {
    console.log("Bussiness Profile error:",err)
...
// to update a bussiness profile 
export async function updateBussinessProfile(req,res){
  try {
    const {userId}=getAuth(req)

    if(!userId){
      return res.status(401).json({success:false,message:"Authentication Rfequired"})
    }
    const {id}=req.params
    const body =req.body ||{}
    const fileUrls=uploadedFilesToUrls(req)

    const existing =await BussinessProfile.findById(id)
    ...
    const updated =await BussinessProfile.findByIdAndUpdate(id,update,{
      new:true,
      runValidators:true
    })
    ...
    console.log("UPDATEBussiness Profile error:",err)
    return res.status(500).json({
      success:false,
      message:"Server Error"
    })
...
// To get my Bussiness Profile
export async function getMyBussinesProfile(req,res){
  try {
    const {userId}=getAuth(req)
    if(!userId){
      return res.status(401).json({success:false,message:"Authentication Required"})
    }
    const profile =await BussinessProfile.findOne({owner:userId}).lean()
    if(!profile){
    ...
    console.log("Get My Bussiness Profile error:",error)
    return res.status(500).json({
      success:false,
      message:"Server Error"
    })
